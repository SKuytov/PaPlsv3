...
const MaintenanceScanner = ({ onLogout, technicianName, technicianId }) => {
   ...
   const processBarcode = async (code, source) => {
      ...
      try {
          ...
          // SUCCESS: Part found
          console.log(`[MaintenanceScanner] Part found successfully: ${part.name}`);
          const scanData = {
              barcode: code,
              part,
   ...
   const handleScan = useCallback((code, source = 'hid') => {
      console.log(`[MaintenanceScanner] handleScan called: ${code} from ${source}, isProcessing=${isProcessing}`);
      
      if (isProcessing) {
          console.warn(`[MaintenanceScanner] Scan blocked - already processing`);
          return;
      }
      
      console.log(`[MaintenanceScanner] Received: ${code}`);
      playBeep('success');
      setIsProcessing(true); // Lock via STATE

      scanQueueRef.current.push({ code, source });
      console.log(`[MaintenanceScanner] Added to queue, length=${scanQueueRef.current.length}`);
   ...
   useEffect(() => {
      if (scanStep === 'scan' && isProcessing) {
         console.log('[MaintenanceScanner] Returning to scan mode, unlocking...');
         setIsProcessing(false);
      }
   }, [scanStep]);
   ...
   useEffect(() => {
      ...
      console.log('[MaintenanceScanner] Setting up HID listener');
      ...
      const handleKeyDown = (event) => {
         ...
         console.log(`[MaintenanceScanner] Buffer: "${hidBufferRef.current}"`);
         ...
         hidTimeoutRef.current = setTimeout(() => {
            if (hidBufferRef.current.length > 2) {
               console.log(`[MaintenanceScanner] Timeout -> processing: ${hidBufferRef.current}`);
               handleScan(hidBufferRef.current, 'hid');
            }
            hidBufferRef.current = '';
         }, HID_SCAN_TIMEOUT_MS);
   ...
   const safeStopVideoTracks = () => {
      if (videoRef.current?.srcObject) {
         try {
            ...
            console.error('[MaintenanceScanner] Error stopping video:', e);
         }
      }
   };
               ...
               {scanStep === 'scan' && (
                  <Tabs value={mode} onValueChange={setMode} className="w-full">
                     ...
                     <TabsContent value="hid" className="mt-0">
                        <div className="space-y-4 p-8 bg-slate-50 rounded-lg border">
                           <div className="text-center space-y-2">
                              <p className="text-sm text-slate-600 font-medium">External Barcode Scanner Ready</p>
                              <p className="text-xs text-slate-500">Zebra LS2208 - Point and scan</p>
                           </div>
                     ...
                     <TabsContent value="manual" className="mt-0">
                        <form onSubmit={handleManualSubmit} className="space-y-4 p-8 bg-slate-50 rounded-lg border">
               ...
               {scanStep === 'menu' && activePart && (
                  <div className="animate-in fade-in zoom-in-95 duration-200">
                     ...
                     <div className="grid grid-cols-1 gap-3 mb-4">
                        <Button size="lg" className="h-16 text-lg font-semibold bg-red-600 hover:bg-red-700" onClick={() => setScanStep('transaction')}>
               ...
               {scanStep === 'transaction' && activePart && (
                  <div className="animate-in slide-in-from-right-4 duration-200">
                     <div className="flex items-center mb-4">
                        ...
                        <h3 className="ml-auto font-semibold text-lg">Register Usage</h3>
                     </div>
                     <PartSummary part={activePart.part} />
                     <div className="space-y-5 bg-white p-1">
                        ...
                        <div className="space-y-2">
                           <Label>Notes</Label>
                           <Textarea placeholder="Why?" value={txNotes} onChange={(e) => setTxNotes(e.target.value)} className="resize-none h-24" />
                        </div>
