-- Blade Serial Number Counter System
-- Migration: 006-blade-serial-tracking.sql
-- Created: 2026-01-12
...
-- ============================================
-- This table tracks the next serial number to assign for each blade type
-- Ensures sequential numbering even across multiple purchase orders
-- Example: Type 4 (B4) - current_counter = 105 means next blade will be B400106
...
-- ============================================
-- Tracks when new blades are purchased and their serial number ranges
-- Helps track inventory and maintenance history for purchase batches
...
-- ============================================
-- Tracks when blades are retired/decommissioned
-- Important for warranty, recalls, and historical analysis
...
-- ============================================
-- Real-time inventory view for each blade type
-- Helps with ordering decisions and stock management
...
-- blade_serial_counter: Read for all, write for admin/supervisor
create policy "blade_serial_counter_select_authenticated" on blade_serial_counter
  for select
  using (auth.role() = 'authenticated');

create policy "blade_serial_counter_update_admin" on blade_serial_counter
  for update
  using (
    exists (
...
-- blade_purchase_order: Read for all, write for supervisor/admin
create policy "blade_purchase_order_select_authenticated" on blade_purchase_order
  for select
  using (auth.role() = 'authenticated');

create policy "blade_purchase_order_insert_supervisor" on blade_purchase_order
  for insert
  with check (
    exists (
      select 1 from user_roles
...
create policy "blade_purchase_order_update_supervisor" on blade_purchase_order
  for update
  using (
    exists (
      select 1 from user_roles
...
-- blade_retirement_record: Read for all, write for supervisor/technician
create policy "blade_retirement_record_select_authenticated" on blade_retirement_record
  for select
  using (auth.role() = 'authenticated');

create policy "blade_retirement_record_insert_technician" on blade_retirement_record
  for insert
  with check (
    exists (
      select 1 from user_roles
...
-- ============================================
-- This assumes blade types already exist from previous migration
-- Adjust the serial prefixes according to your machine types
