...
-- Migration: 005-blade-lifecycle-tracking.sql
-- Created: 2026-01-12
...
-- blade_types: Read access for all authenticated users, write for admin/supervisor
...
create policy "blade_types_update_admin" on blade_types
  for update
  using (
    exists (
      select 1 from user_roles
...
-- blades: Read access for all, write for supervisor/technician
create policy "blades_select_authenticated" on blades
  for select
  using (auth.role() = 'authenticated');

create policy "blades_insert_supervisor_technician" on blades
  for insert
  with check (
    exists (
      select 1 from user_roles
...
create policy "blades_update_supervisor_technician" on blades
  for update
  using (
    exists (
      select 1 from user_roles
...
-- blade_usage_logs: Write for technician/operator
create policy "blade_usage_logs_select_authenticated" on blade_usage_logs
  for select
  using (auth.role() = 'authenticated');

create policy "blade_usage_logs_insert_technician" on blade_usage_logs
  for insert
  with check (
    exists (
      select 1 from user_roles
      where user_id = auth.uid()
      and role in ('admin', 'supervisor', 'technician', 'operator')
    )
  );

-- blade_sharpening_history: Write for technician
create policy "blade_sharpening_select_authenticated" on blade_sharpening_history
  for select
  using (auth.role() = 'authenticated');

create policy "blade_sharpening_insert_technician" on blade_sharpening_history
  for insert
  with check (
    exists (
      select 1 from user_roles
...
-- blade_maintenance_logs: Write for supervisor/technician
create policy "blade_maintenance_select_authenticated" on blade_maintenance_logs
  for select
  using (auth.role() = 'authenticated');

create policy "blade_maintenance_insert_technician" on blade_maintenance_logs
  for insert
  with check (
    exists (
      select 1 from user_roles
...
-- blade_alerts: Read for all, write for system/admin
create policy "blade_alerts_select_authenticated" on blade_alerts
  for select
  using (auth.role() = 'authenticated');

create policy "blade_alerts_update_admin" on blade_alerts
  for update
  using (
    exists (
      select 1 from user_roles
